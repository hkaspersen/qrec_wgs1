...
# Introduces unique ID's for each sample
fix_ref_id <- function(df) {
  df <- df %>%
...
if (length(mut_genes) == 1) {
  if (mut_genes == "ALL") {
    ...
    mut_report_fixed <- fix_ref_id(mut_report)
    }
} else {
...
if (length(ac_genes) == 1) {
  if (ac_genes == "ALL") {
    ...
    acquired_report_fixed <- fix_ref_id(acquired_report)
  }
} else {
...
mut_quant_fixed <- fix_ref_id(mut_quant)
...
```{r}
acquired_report_fixed %>%
  left_join(mut_quant_fixed[c("gyrA","gyrB","parC","parE","id")], by = "id") %>%
  left_join(mut_report_fixed[c("marR","id")], by = "id") %>%
...
```{r}
total_report <- mut_report_fixed %>%
  left_join(acquired_report_fixed, by = "id") %>%
...
genes <- c(names(mut_report_fixed), names(acquired_report_fixed))
