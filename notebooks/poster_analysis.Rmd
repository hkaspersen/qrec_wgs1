...
## Significance test for the percent presence of qnrB19 and qnrS1 between animal species
```{r message=FALSE, warning=FALSE}
specific_qnr <- qnr_data %>%
  filter(key %in% c("qnrB19", "qnrS1")) %>%
...
p_values <- lapply(specific_qnr, function(x) do_chisq_test(x)) %>%
  bind_rows(.id = "gene")
